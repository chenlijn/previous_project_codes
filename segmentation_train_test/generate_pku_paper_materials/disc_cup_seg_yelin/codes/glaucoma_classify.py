...
if __name__ == '__main__':
    ...
    for img_type in img_types:
        ...
        disc_path = image_path_root + img_type + '/disc_deeplabv3/'
        cup_path = image_path_root + img_type + '/cup_deeplabv3/'
        ...
        for imgname in imgnames:
            if imgname.endswith('.png'):
                ...
                det_img = cv2.imread(os.path.join(det_path, imgname))
                ...
                if cup2disc_ratio > threshold + space*2:
                    #if img_type == 'healthy':
                    ...
                    decisions.append(3)
                elif cup2disc_ratio > threshold + space:
                    decisions.append(2)

                elif cup2disc_ratio > threshold:
                    decisions.append(1)
        ...
        df = DataFrame({"image name": imgnames, "cup to disc ratio": ratio_list, "class result": decisions, "":["" for i in range(len(imgnames))], "":["" for i in range(len(imgnames))]})
        df = df[["image name", "cup to disc ratio", "class result", "", ""]]
        img_num = len(ratio_list)
        df.loc[img_num] = ['mean ratio', np.mean(ratio_list), '', '', '']
        df.loc[img_num+1] = ['', '', '', '', '']
        df.loc[img_num+2] = ['threshold', threshold, threshold+space, threshold+2*space, '']
        df.loc[img_num+3] = ['total num', 'healthy num', 'light num', 'mid num', 'serious num']
        df.loc[img_num+4] = [img_num, decisions.count(0), decisions.count(1), decisions.count(2), decisions.count(3)]
        df.loc[img_num+5] = ['rate', decisions.count(0)/float(img_num), decisions.count(1)/float(img_num), decisions.count(2)/float(img_num), decisions.count(3)/float(img_num)]
