...
def main(image_name, image_path, mask_path, save_path, ddtr, unet_cup_seg): # lijian
#def main(image_name, image_path, save_path, ddtr, cup_seg):
	...
	mask = cv2.imread(os.path.join(mask_path,image_name))
        ...
        print image.shape
	img_disc, res_mask = ddtr.forward_net(image, mask)
        cv2.imwrite(os.path.join(save_path+'/det', image_name.split('.')[0]+'.png'), img_disc)
        #cv2.imwrite(os.path.join(save_path+'/det_mask', image_name.split('.')[0]+'.png'), res_mask)
...
if __name__ == '__main__':
        ...
        image_path_root = '/mnt/lijian/mount_out/data/pku_glaucoma_201712_label_consistency_check/sorted_data/images/'
        mask_path_root = '/mnt/lijian/mount_out/data/pku_glaucoma_201712_label_consistency_check/sorted_data/masks/'
        save_path_root = '/mnt/lijian/mount_out/data/pku_glaucoma_201712_label_consistency_check/sorted_data/predictions/'
        ...
        folders = ['Annotation-bing', 'Annotation-xing', 'Annotation-kang', 'Annotation-wei']
        img_types = ['light', 'mid']
	...
	#cup_seg = disc_cup_seg(cfg.params.cup_seg_proto(), cfg.params.cup_seg_weight(),cfg.params.get_gpu_id_0())
        ...
        for folder in folders:
            for img_type in img_types:
                image_path = image_path_root + folder + '/' + img_type
                mask_path = mask_path_root + folder + '/' + img_type
                save_path = save_path_root + folder + '/' + img_type
                if not os.path.isdir(save_path + '/cup/'):
                    os.mkdir(save_path + '/cup/')
                if not os.path.isdir(save_path + '/det/'):
                    os.mkdir(save_path + '/det/')
                imgnames = os.listdir(image_path)
                for image_name in imgnames:
                    if image_name.endswith('.png'):
                    #if image_name.endswith('.jpg'):
                        #main(image_name, image_path, save_path, ddtr, disc_seg)
                        main(image_name, image_path, mask_path, save_path, ddtr, unet_cup_seger)
