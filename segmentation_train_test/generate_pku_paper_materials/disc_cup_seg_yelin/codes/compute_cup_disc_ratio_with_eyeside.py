...
if __name__ == '__main__':
    ...
    image_path_root = '/root/mount_out/data/original_glaucoma_data/sorted_data/glaucoma_cls_test_set/splits/'
    ...
    img_types = ['online_normal_test', 'online_abnormal_test',  'healthy', 'light', 'mid', 'serious']
    #img_types = ['online_abnormal_test']

    for img_type in img_types:
        show_path = 'bad_case/{}'.format(img_type)
        show_good_path = 'good_case/{}'.format(img_type)
        bad_case_path = '../bad_case/'
        eyeside_txt = image_path_root + 'det/' + img_type + '_with_path_eye_side.txt'
        eyeside_dict = get_eyeside_dict(eyeside_txt)

        record_file = "csv/" + img_type + '_cup_to_disc_ratio.csv'

        det_path = image_path_root + '/det/' + img_type 
        ...
        for idx, imgname in enumerate(imgnames):
            if imgname.endswith('.png'):
                ...
                if glaucoma:
                    if img_type == 'healthy' or img_type == 'online_normal_test':
                        # to draw ans store the intermediate results
                        img = cv2.imread(os.path.join(det_path, imgname.split('.')[0]+'.jpg'))
                        show_img = draw_seg_and_centroid(np.copy(img), disc, cup)
                        show_img = get_isnt(disc, cup, eyeside=eyeside, img=show_img)
                        ...
                        put_text2 = 'cup_to_disc_ratio: {:6.4f}'.format(ratio)
                        cv2.putText(show_img, put_text2, (30, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0),2)
                        cv2.imwrite(os.path.join(show_good_path, imgname), show_img)
                    decisions.append(1)
                    ...
                    ratio_list.append(ratio)
                    #ratio_list.append('-')
                    if img_type != 'healthy' and img_type != 'online_normal_test':
                        # to draw ans store the intermediate results
                        img = cv2.imread(os.path.join(det_path, imgname.split('.')[0]+'.jpg'))
                        show_img = draw_seg_and_centroid(np.copy(img), disc, cup)
                        show_img = get_isnt(disc, cup, eyeside=eyeside, img=show_img)
