...
def split_disc_by_mask(image, mask, mask_value):
    """split the double discs by mask"""

    rows, cols = np.where(mask==mask_value)
    mean_x = int(np.mean(cols))
    mean_y = int(np.mean(rows))

    mask = np.where(mask==mask_value, 1, 0)
    ...
    ##if center_img_area.mean() < 50 and center_area.sum()< (2*wid)**2:
    #if center_area.sum()< (2*wid)**2:
    #   if mean_x < center_x:
    #        #return image[:, :center_x, :], mask[:, :center_x]
    #        return image[:, :center_x+10*wid, :], mask[:, :center_x+10*wid]
    #   else:
    #        #return image[:, center_x:, :], mask[:, center_x:]
    #        return image[:, center_x-10*wid:, :], mask[:, center_x-10*wid:]
    #else:
